...
void Flow::onRequestFirstLineReceived(const system::error_code &err, std::size_t bytes) {
    if (err.value()) {
        std::cerr << "Error in async_read_until (onRequestFirstLineReceived): " << err.value() << " " << err.message() << std::endl;
        destroy();
        return;
    }
    ...
    std::cout << *(static_cast<const char *>(_request.data().data()));
    ...
    _request.consume(_request.size()); //todo
    asio::async_read_until(inSocket(), _request, "\r\n\r\n",
                           std::bind(&Flow::onRequestHeadersReceived, this,
                                     std::placeholders::_1, std::placeholders::_2));
}

void Flow::onRequestHeadersReceived(const system::error_code &err, std::size_t bytes) {
    if (err.value()) {
        std::cerr << "Error in async_read_until (onRequestHeadersReceived): " << err.value() << " " << err.message() << std::endl;
        destroy();
        return;
    }
    ...
    //todo: читать до Content-Length или до EOF
    asio::async_read(inSocket(), _request,
                     std::bind(&Flow::onRequestBodyReceived, this,
                               std::placeholders::_1, std::placeholders::_2));
}
